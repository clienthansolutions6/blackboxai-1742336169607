...
$query = "SELECT bk.*, b.kode_barang, b.nama_barang, s.nama_satuan, u.nama_lengkap as petugas
...
<div class="bg-white shadow-sm border-b mb-6">
    <div class="px-4 py-4 sm:px-6 flex items-center justify-between">
        <h2 class="text-xl font-semibold text-gray-800">Data Barang Keluar</h2>
        <div class="flex space-x-2">
            <a href="?module=barang-keluar&action=export" class="bg-green-500 hover:bg-green-600 text-white py-2 px-4 rounded-lg text-sm font-medium flex items-center">
                <i class="fas fa-file-excel mr-2"></i> Export Excel
            </a>
            <a href="?module=barang-keluar&action=form_entri" class="bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded-lg text-sm font-medium flex items-center">
...
<div class="bg-white rounded-lg shadow mb-6 p-4">
    <form action="" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <input type="hidden" name="module" value="barang-keluar">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Lokasi Asal</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        <?php echo date('d/m/Y', strtotime($data['tanggal_keluar'])); ?>
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        <?php 
                        $query_lokasi = "SELECT l.nama_lokasi 
                                       FROM lokasi l 
                                       WHERE l.id_lokasi = {$data['id_lokasi']}";
                        $result_lokasi = query($query_lokasi);
                        $lokasi = fetch_assoc($result_lokasi);
                        echo $lokasi['nama_lokasi'];
                        ?>
                    </td>
                    <td class="px-6 py-4 text-sm text-gray-900"><?php echo $data['tujuan']; ?></td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <a href="?module=barang-keluar&action=form_ubah&id=<?php echo $data['id_barang_keluar']; ?>" 
